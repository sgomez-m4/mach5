...
# =====================================================================
CIKS_A_MONITOREAR = [
 '0000006201','0000027904','0000100517','0000766421','0000092380','0001158463','0001670076','0000793733','0000810332','0001846084'
]
...
def obtener_historial_ids():
    """Descarga el historial de IDs de reportes ya procesados desde Cloud Storage."""
    bucket = storage_client.bucket(BUCKET_NAME)
    blob = bucket.blob("historial_reportes.txt")
    if blob.exists():
...
def obtener_urls_documentos(url_index):
    ...
    las URLs de TODOS los documentos asociados (el 8-K principal + exhibits,
    p.ej. comunicados de prensa en EX-99.1), que es donde suele estar la
    información real sobre flota (compras, leasing, retiros, entregas, etc.).
    """
    urls = []
    try:
...
def extraer_texto_completo_filing(url_index):
    """
    Combina el texto del filing principal con el de todos sus documentos
    adjuntos (exhibits), respetando un límite total de caracteres para
    no exceder el contexto enviado a Gemini.
    """
...
def generar_analisis_gemini(texto_reporte, titulo_reporte):
    """
    Envía el texto combinado (filing + adjuntos) a Gemini y obtiene una
    ...
    prompt = f"""
    Eres un consultor aeronáutico experto en gestión de flota. Analiza la siguiente presentación 8-K ante la SEC,
    INCLUYENDO el documento principal y todos sus documentos adjuntos (exhibits, comunicados de prensa, etc.),
    ya que la información relevante sobre flota suele estar en los adjuntos y no en el cuerpo principal del 8-K.
...
def enviar_correo_alerta(titulo, link, cik, analisis):
    ...
    if es_error:
        ...
        """
        encabezado = "⚠️ Monitoreo Automático SEC: Error de Análisis — Revisión Manual Requerida"
        asunto = f"⚠️ REVISIÓN MANUAL (error de análisis): {titulo[:40]}... (CIK: {cik})"
    else:
        banner = ""
        encabezado = "✈️ Monitoreo Automático SEC: Posible Impacto en Flota Detectado"
        asunto = f"✈️ Impacto en Flota Detectado: {titulo[:40]}... (CIK: {cik})"
    ...
    <hr/>
    <p style="font-size: 11px; color: #777;">Alerta automatizada generada desde GCP Cloud Functions. Se analizaron el documento principal y sus adjuntos.</p>
    """
...
def monitorear_sec_event(request):
    """Punto de entrada HTTP de la Cloud Function."""
    print("Iniciando verificación periódica de reportes 8-K en la SEC...")
    ...
    for cik in CIKS_A_MONITOREAR:
        ...
        try:
            ...
            for entrada in feed.entries:
                ...
                if "8-K" not in categoria:
                    continue
                if reporte_id in historial_ids:
                    continue

                print(f"🔥 Nuevo reporte 8-K detectado para CIK {cik}. Procesando (incluyendo adjuntos)...")
    ...
    return {
        "status": "success",
        "filings_8k_procesados": filings_procesados,
